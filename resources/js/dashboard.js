document.addEventListener("DOMContentLoaded", function () {
    ...
    let selectedUserIds = [];
    let lastEnvironmentalData = null;
    ...
    async function fetchAndUpdateStats() {
        ...
        try {
            ...
            // Update Environmental Charts with real data
            if (json.environmentalData) {
                lastEnvironmentalData = json.environmentalData;
                renderTemperatureChart(json.environmentalData.temperature);
                renderLightChart(json.environmentalData.light);
                renderHumidityChart(json.environmentalData.humidity);
            }
        ...
        const plotData = [
            {
                ...
                labels: labels,
                marker: {
                    colors: labels.map((l) => stateColors[l] || COLORS[0]),
                },
                type: "pie",
                hole: 0.4,
    ...
    function renderTemperatureChart(data) {
        const chartData =
            data || generateEnvironmentalData(22, 0.5, 1.5, 18, 25);
        const mainTrace = {
            x: chartData.x,
            y: chartData.y,
            mode: "lines",
    ...
    function renderLightChart(data) {
        const chartData =
            data || generateEnvironmentalData(400, 30, 80, 300, 750);
        const trace = {
            x: chartData.x,
            y: chartData.y,
            fill: "to zero",
    ...
    function renderHumidityChart(data) {
        const chartData = data || generateEnvironmentalData(45, 1, 0.5, 35, 55);
        const trace = {
            x: chartData.x,
            y: chartData.y,
            mode: "lines",
    ...
    function setupEnvironmentalChartPagination() {
        ...
        const renderFunctions = {
            tempPlot: () =>
                renderTemperatureChart(lastEnvironmentalData?.temperature),
            lightPlot: () => renderLightChart(lastEnvironmentalData?.light),
            humidityPlot: () =>
                renderHumidityChart(lastEnvironmentalData?.humidity),
        };
