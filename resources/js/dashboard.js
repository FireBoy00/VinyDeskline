...
const COLORS = ['#004F6E', '#0485B9', '#66B2D0', '#5B686E', '#C6DAE2', '#86A7B8', '#A9C4D3', '#396A87']; 
...
function renderSitStandTimelineChart() {
    ...
    for (let i = 0; i < NUM_USERS; i++) {
        ...
        const trace = {
            x: userData.x, y: userData.y, mode: 'lines', name: `User ${i + 1}`,
    ...
    const layout = {
        ...
        xaxis: { title: 'Time of Day', type: 'date', tickformat: '%I:%M %p', range: [new Date().setHours(7, 30), new Date().setHours(16, 30)] },
        yaxis: { title: 'Height (mm)', tickvals: [700, 1100], ticktext: ['(Base 700)', '(Base 1100)'], range: [650, 1350] },
        margin: { t: 50, b: 50, l: 80, r: 20 },
        plot_bgcolor:'#dce5e9', paper_bgcolor: '#dce5e9', showlegend: true,
        legend: { orientation: 'h', x: 0.5, y: 1.15, xanchor: 'center' }
    };

    if (window.Plotly) {
        window.Plotly.newPlot(CHART_CONTAINER_IDS.timeline, traces, layout, { responsive: false, displayModeBar: false });
    }
}
...
function renderStandingPercentageChart() {
    ...
    if (window.Plotly) {
        window.Plotly.newPlot(CHART_CONTAINER_IDS.standing, data, layout, { responsive: true, displayModeBar: false });
    }
}
...
function renderDeskStateOverviewChart() {
    ...
    if (window.Plotly) {
        window.Plotly.newPlot(CHART_CONTAINER_IDS.deskState, data, layout, { responsive: true, displayModeBar: false });
    }
}
...
function generateDailyUsageDuration() {
    ...
    days.forEach(() => {
        const totalWorkHours = Math.floor(Math.random() * (400 - 200 + 1)) + 200; 
        const standingHours = Math.round((Math.random() * 80 + 40) * 10) / 10; 
        const sittingHours = Math.round((totalWorkHours - standingHours) * 10) / 10;
...
function renderDailyUsageDurationChart() {
    ...
    if (window.Plotly) {
        window.Plotly.newPlot(CHART_CONTAINER_IDS.dailyUsage, traces, layout, { responsive: true, displayModeBar: false });
    }
}
...
function renderLightChart() {
    ...
    const layout = {
        ...
        margin: { t: 50, b: 50, l: 80, r: 20 },
        plot_bgcolor:'#dce5e9', paper_bgcolor: '#dce5e9', showlegend: false
    };

    if (window.Plotly) {
        window.Plotly.newPlot(CHART_CONTAINER_IDS.light, [trace], layout, { responsive: true, displayModeBar: false });
    }
}
...
function renderHumidityChart() {
    ...
    if (window.Plotly) {
        window.Plotly.newPlot(CHART_CONTAINER_IDS.humidity, [trace], layout, { responsive: true, displayModeBar: false });
    }
}
